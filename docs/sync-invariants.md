...
| #   | Invariant                                                                                                      | Verified by                                                                                            |
| --- | -------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| R1  | A template has no financial effect. Only a generated transaction moves a balance, report or budget.            | `test/recurring/recurring-engine.test.ts`                                                              |
| R2  | A generated transaction is an ordinary expense or income, built by the transaction service's own rules.        | `test/recurring/recurring-engine.test.ts`                                                              |
| R3  | Two devices generating the same date converge on one occurrence and one transaction.                           | `test/sync/recurring-sync.test.ts` — "converges two offline generations"                               |
| R4  | An occurrence's identity is derived from its template and date; a generated transaction's from its occurrence. | `test/recurring/recurring-identity.test.ts`, `verifySyncIntegrity`, `test/sync/recurring-sync.test.ts` |
| R5  | Generation is atomic: no handled date without its transaction, no generated transaction without its date.      | `test/recurring/recurring-engine.test.ts` — "atomic generation"                                        |
| R6  | Generated wins over skipped, in every upload order and on download.                                            | `test/sync/recurring-sync.test.ts`, `supabase/tests/recurring.sql`                                     |
| R7  | A handled date is never offered again — including after its transaction is deleted.                            | `test/recurring/recurring-engine.test.ts`                                                              |
| R8  | Deleting a template, or editing it, never touches a transaction it already generated.                          | `test/recurring/recurring-engine.test.ts`, `test/sync/recurring-sync.test.ts`                          |
| R9  | What is due is derived from the schedule and the occurrences, never stored, synchronized or backed up.         | `test/backup/recurring-backup.test.ts`, `test/sync/recurring-sync.test.ts`                             |
| R10 | Working out what is due never reads the transactions table.                                                    | `test/recurring/recurring-performance.test.ts`                                                         |
| R11 | Nothing is generated by a migration, a restore, a download or the passage of time — only by a call.            | `test/recurring/recurring-migration.test.ts`, `test/sync/boundaries.test.ts`                           |

## Investments

| #   | Invariant                                                                                                                                    | Verified by                                                                                                           |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| V1  | Holdings, cost basis, gains and values are never stored, transported or backed up; every device replays trades.                              | `test/sync/investment-sync.test.ts`, `test/backup/investment-backup.test.ts`                                          |
| V2  | A trade's cash moves once: balances read only its linked transaction, never the trade.                                                       | `test/investments/investment-domain.test.ts` — "counts the cash exactly once"                                         |
| V3  | A trade, its cash transaction and both queue entries commit together or not at all.                                                          | `test/investments/investment-domain.test.ts` — "commit together or not at all"                                        |
| V4  | Buying, selling and investment fees never change Income, Expense, Savings or a budget; a dividend is Investment Return income.               | `test/investments/investment-domain.test.ts`                                                                          |
| V5  | No asset's live history ever sells more than it held at any point — locally, on download, in a restore, or in the cloud.                     | `investment-domain.test.ts`, `investment-sync.test.ts`, `investment-backup.test.ts`, `supabase/tests/investments.sql` |
| V6  | Two devices selling the same units offline: the cloud refuses the second upload, and the second device refuses the first's sale on download. | `test/sync/investment-sync.test.ts` — "never applies the second of two offline sales"                                 |
| V7  | Trades replay in trade date, `createdAt`, `syncId` order on every device and in the cloud's guard.                                           | `test/investments/investment-math.test.ts`, `supabase/tests/investments.sql`                                          |
| V8  | An unpriced position has an unknown value, never zero; no figure adds two currencies.                                                        | `test/investments/investment-domain.test.ts` — valuation                                                              |
| V9  | Investment cash cannot exist without its trade, locally, in the cloud or in a backup.                                                        | `test/sync/pull-sync.test.ts`, `supabase/tests/investments.sql`, `test/backup/investment-backup.test.ts`              |
| V10 | Quantity is integer 10^-8 units and every product is BigInt; a figure that does not fit is refused, not rounded.                             | `test/investments/investment-math.test.ts`                                                                            |
